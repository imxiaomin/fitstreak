...
test('FR-02/04 create plan, check in, stats, edit and archive',async({page})=>{
 ...
 await page.locator('.dialog .field').filter({hasText:'计划名称'}).locator('input').fill('E2E workout');await page.getByRole('button',{name:'保存',exact:true}).click();await expect(page.getByText('E2E workout').first()).toBeVisible();
 const row=page.locator('.session-row').filter({hasText:'E2E workout'});await row.getByRole('button',{name:'去打卡'}).click();await page.locator('.dialog input').first().fill('42');await page.getByRole('button',{name:'完成打卡',exact:true}).click();await expect(row.getByRole('button',{name:'已完成'})).toBeDisabled();
 ...
 await page.locator('.sidebar').getByRole('button',{name:'训练计划',exact:true}).click();const card=page.locator('.plan-card').filter({hasText:'E2E workout'});await card.getByRole('button',{name:'编辑',exact:true}).click();await page.locator('.dialog .field').filter({hasText:'计划名称'}).locator('input').fill('Updated E2E');await page.getByRole('button',{name:'保存',exact:true}).click();
...
test('FR-08/09 knowledge translation and profile persistence',async({page})=>{
 ...
 await page.locator('.sidebar').getByRole('button',{name:'Profile',exact:true}).click();await page.locator('.preferences input').first().fill('Lin');await page.getByRole('button',{name:'Save changes',exact:true}).click();await expect(page.locator('.profile-card').getByText('Lin',{exact:true})).toBeVisible();await page.reload();await expect(page.getByText('A little stronger, every day.')).toBeVisible();
});
