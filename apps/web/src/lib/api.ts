...
export function request<T=any>(path:string,method:'GET'|'POST'|'PATCH'|'DELETE'='GET',data?:unknown):Promise<T>{
 return new Promise((resolve,reject)=>uni.request({url:base+path,method,data,timeout:12000,header:{'Content-Type':'application/json',...(session.token?{Authorization:`Bearer ${session.token}`}:{})},success:r=>{const body=r.data as any;if(r.statusCode>=200&&r.statusCode<300)resolve(body.data);else{if(r.statusCode===401)session.token='';reject(new Error(body?.error?.code??'INTERNAL_ERROR'));}},fail:()=>reject(new Error('error'))}));
}
