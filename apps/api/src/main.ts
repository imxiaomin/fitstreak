...
const app=await buildApp({secret:process.env.JWT_SECRET??randomBytes(48).toString('hex'),demo:!production && process.env.DEMO_MODE!=='false',logger:true,corsOrigin:process.env.CORS_ORIGIN,wechat:{appid:process.env.WECHAT_APP_ID??'',secret:process.env.WECHAT_APP_SECRET??''}});
